                ...
                <article class="post">
                            ...
                            @if($post->categories->count() > 0)
                                <span class="post-categories ms-3">
                                    ...
                                    @foreach($post->categories as $category)
                                        <a href="{{ url('category/' . $category->slug) }}" class="category-link">{{ $category->name }}</a>{{ !$loop->last ? ', ' : '' }}
                                    @endforeach
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="post-share mt-4">
                        ...
                        <div class="social-share">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url('blog/' . $post->slug)) }}" target="_blank" class="btn btn-primary me-2">
                                <i class="fab fa-facebook-f"></i> Facebook
                            </a>
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(url('blog/' . $post->slug)) }}&text={{ urlencode($post->title) }}" target="_blank" class="btn btn-info me-2">
                                <i class="fab fa-twitter"></i> Twitter
                            </a>
                            <a href="https://wa.me/?text={{ urlencode($post->title . ' ' . url('blog/' . $post->slug)) }}" target="_blank" class="btn btn-success">
                                ...
                                <div class="col-md-4 mb-4">
                                    <div class="card h-100">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <a href="{{ url('blog/' . $related_post->slug) }}" class="btn btn-sm btn-primary">Baca Selengkapnya</a>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                            ...
                            <ul class="list-group">
                                @foreach($recent_posts as $recent_post)
                                    <li class="list-group-item">
                                        ...
                                        <small class="text-muted d-block">{{ $recent_post->created_at->format('d M Y') }}</small>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
